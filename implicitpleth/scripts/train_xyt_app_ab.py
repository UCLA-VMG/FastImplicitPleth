...
def main(args):
    ...
    app_enc = tcnn.Encoding(config["encoding"]["input_dims"], config["encoding"])
    app_net = tcnn.Network(app_enc.n_output_dims, config["network"]["output_dims"], config["network"])
    appearance_model = torch.nn.Sequential(app_enc, app_net)
    ...
    appearance_model.to(args.pleth_device)
    ...
    for epoch in range(start_epoch,epochs+1):
        ...
        for count, item in enumerate(dloader):
            ...
            appearance_output = appearance_model(loc)
        ...
        if args.trace["folder"] is not None:
            if epoch >= args.trace["trace_epoch"]:
                with torch.no_grad():
                    ...
                    appearance_output = appearance_model(trace_loc)
